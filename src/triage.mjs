...
//   Never actionable (auto-excluded, summarized only):
//     - UNKNOWN guest starting choice (blank tags / unrecognized tags / host
//       simply didn't record it) — regardless of match status
...
//     2. Half/Half mixed-menu policy exceptions
//     3. one strong Toast candidate that failed the automatic threshold
//     4. detected table transfers needing attribution confirmation
//     5. items a manager reopened
//   Excluded records never touch conversion (numerator or denominator).
//
...
export const KIND = {
  ...
  MIXED: 'MIXED_MENU',              // Half/Half policy exception
...
const KIND_PRIORITY = { [KIND.CONFLICT]: 1, [KIND.REOPENED]: 2, [KIND.MIXED]: 3, [KIND.MATCH]: 4, [KIND.TRANSFER]: 5 };
...
export function classifyRecord(r) {
  ...
  if (r.mixedMenuException) return { actionable: true, kind: KIND.MIXED };
  if (r.intent === 'REVIEW_REQUIRED') return { actionable: true, kind: KIND.CONFLICT };
  if (r.intent === 'UNKNOWN' || r.intent == null) {
    // no recorded starting choice — never management work, never conversion
  ...
  if (r.matchStatus === 'ambiguous' && r.matchedOrderGuid
 ...
 * commission), then conflicting tags, reopened, mixed-menu, likely matches,
 * transfers; newest date first inside a group.
 */
export function triageIntents(rows) {
  const actionable = [];
  const excluded = {
    unmarked: 0,             // no recorded starting choice (auto-excluded)
...
export function exclusionSummaryLines(excluded) {
  ...
  if (excluded.unmarked) {
    lines.push(`${excluded.unmarked} visit${excluded.unmarked === 1 ? '' : 's'} did not have a recorded starting choice and ${excluded.unmarked === 1 ? 'was' : 'were'} excluded automatically.`);
  }
  if (excluded.markedNotConnected) {
    lines.push(`${excluded.markedNotConnected} visit${excluded.markedNotConnected === 1 ? '' : 's'} had a recorded choice but no reliable Toast table and ${excluded.markedNotConnected === 1 ? 'was' : 'were'} excluded automatically.`);
  }
  return lines;
}
